...
class Settings:
    ...
    async def format(self):
        embed = discord.Embed(
            title="Settings", description=self.guild.name, color=discord.Color.random())

        embed.set_thumbnail(url=self.guild.icon_url)
        embed.set_footer(
            text="Usage: set setting_name value")
        ...
        for key in self.config.keys():
            ...
            if self.config.get(key) == "" or self.config.get(key) is None:
                ...
                if self.config.get(key) is not None:
                    ...
                    if found is False:
                        embed.add_field(
                            name=key, value="Invalid voice channel", inline=False)
    ...
    async def command_channel(self, setting, value, ctx):
        ...
        for chan in self.guild.text_channels:
            if chan.name.lower() == value.lower():
                ...
                found = True
        if found is False:
            await ctx.send(f"`Error: Channel name not found`\nUsage: set {setting} channelname\nOther options: unset")
            return False
    ...
    async def start_voice_channel(self, setting, value, ctx):
        ...
        for vc in self.guild.voice_channels:
            if vc.name.lower() == value.lower():
                ...
                found = True
        if found is False:
            await ctx.send(f"`Error: Voice channel name not found`\nUsage: {''}set {setting} vchannelname\nOther options: unset")
            return False
    ...
    async def user_must_be_in_vc(self, setting, value, ctx):
        if value.lower() == "true":
            ...
            await ctx.send(f"`Error: Value must be True/False`\nUsage: {''}set {setting} True/False")
            return False
    ...
    async def button_emote(self, setting, value, ctx):
        ...
        if emoji is None:
            await ctx.send(f"`Error: Emote name not found on server`\nUsage: {''}set {setting} emotename\nOther options: unset")
            return False
        else:
    ...
    async def default_volume(self, setting, value, ctx):
        try:
            ...
            await ctx.send(f"`Error: Value must be a number`\nUsage: {''}set {setting} 0-100")
            return False
